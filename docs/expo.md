# Build an Expo app (/tutorials/expo)
...
<Callout title="On Windows" type="info">
  Some commands are Bashâ€‘specific. On Windows, use PowerShell/WSL or adapt them.
</Callout>
...
* [Expo docs â€” get started](https://docs.expo.dev/get-started/introduction/)
* [Expo docs â€” set up your environment](https://docs.expo.dev/get-started/set-up-your-environment/)
...
<Steps>
  <Step>
    ...
    <Callout type="info">
      During the SDK 55 transition period, Expo Go on a physical device requires an SDK 54 project. We pin `blank-typescript@sdk-54` to ensure compatibility with Expo Go. See [Expo docs â€” create a project](https://docs.expo.dev/get-started/create-a-project/) for details.
    </Callout>
  </Step>
...
* A status line progressing through **Downloading modelâ€¦**, **Loading modelâ€¦**, and **Running completionâ€¦**
* A short assistant output streaming into the UI
* A final status of **Done âœ…**
        ...
        useEffect(() => {
          ...
          (async () => {
            try {
              ...
              setStatus("Loading model into memoryâ€¦");
          ...
          return () => {
            ...
            // Cleanup: unload the model (donâ€™t clear cache by default).
            // Note: React cleanup canâ€™t be async directly, so we fire-and-forget.
            const id = modelId;
            if (id) {
        ...
        async function handleSend() {
          ...
          try {
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === assistantId
                  ? { ...m, content: `âŒ Error: ${e?.message ?? String(e)}` }
                  : m
              )
            );
          } finally {
            ...
            >
              ...
              <View style={styles.inputRow}>
                <TextInput
                  ...
                  placeholder={modelId ? "Type a messageâ€¦" : "Loading modelâ€¦"}
              ...
              <Text style={styles.hint}>
                Press â€œsend/enterâ€ to submit. Messages are streamed token-by-token.
              </Text>
            </KeyboardAvoidingView>
          </SafeAreaView>
        );
...
On the first run, the model may download from peers (watch the terminal for progress). Once it finishes, type a message and press **Enter** or click **Send** â€” the response should stream into the UI token by token:
